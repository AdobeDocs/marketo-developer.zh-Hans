...
>[!NOTE]
>
>自2026年7月31日起，SOAP API已弃用，不再可用。 将Marketo [REST API](./rest-api/rest-api.md)用于所有新开发。
...
人才是营销自动化的基础。 Marketo将所有非销售代表记录称为潜在客户，无论销售人员是否将其视为潜在客户、潜在客户、嫌疑人或联系人。

潜在客户对象包括标准字段，例如电子邮件、名字和姓氏。 要存储其他信息，您可以添加字段，并且可以按与标准字段相同的方式读取和写入自定义属性。 在Marketo中的&#x200B;**[!UICONTROL Admin]** > **[!UICONTROL Field Management]**&#x200B;下查找完整的字段列表。

Marketo通过id字段唯一标识潜在客户。 在系统外强制执行其他唯一键。
...
项目在一个位置组织营销人员相关的营销工作。 例如，电子邮件营销活动可以是项目。

潜在客户可以执行与项目群关联的多个操作或活动。 此过程称为潜在客户进展。 对于电子邮件促销活动项目，进度可以记录Marketo发送电子邮件的时间、人员打开电子邮件的时间以及人员是否单击链接。

营销活动在项目中用于特定目的和目标。 例如，营销活动可以选择一组潜在客户并发送电子邮件直播。 当商机单击电子邮件营销活动中的链接时，另一个Campaign可以通知销售代表。
...
例如，要分析哪个区域产生的潜在客户最多，请使用值（如Northeast和Southeast）创建自定义“Region”标记类型。 创建“所有者”标记类型，比较哪些项目所有者对创建潜在客户和机会的影响最大。 有关详细信息，请参阅[了解标记](https://experienceleague.adobe.com/zh-hans/docs/marketo/using/product-docs/core-marketo-concepts/programs/working-with-programs/understanding-tags)。
...
例如，名为“所有访问过网站定价页面的潜在客户”的智能列表会随着更多潜在客户访问该页面而继续增长。 有关详细信息，请参阅[Marketo Engage文档](https://experienceleague.adobe.com/zh-hans/docs/marketo/using/home)。
...
在未启用本机CRM集成时，您可以在Marketo中管理销售代表记录及其潜在客户关系。 这些记录包含名称、电子邮件和职务等信息。 当销售代表拥有商机时，您可以使用此信息来过滤和标记。

通过“externalSalesPersonId”字段在潜在客户级别管理与销售代表的关系。 通过[同步潜在客户](https://developer.adobe.com/marketo-apis/api/mapi#operation/syncLeadUsingPOST) API更新此字段。
