...
批量提取API使用与其他Marketo REST API相同的OAuth 2.0身份验证方法。 这需要嵌入有效的访问令牌作为查询字符串参数 `access_token={_AccessToken_}`或作为HTTP标头 `Authorization: Bearer {_AccessToken_}`.
...
批量提取API根据批量提取作业检索的数据的磁盘大小进行计量。 可以通过读取 `fileSize` 导出作业的已完成状态响应中的属性。

每日配额最大为每天500 MB，在潜在客户、活动、项目群成员和自定义对象之间共享。 当超过配额时，在每日配额于午夜重置之前，您无法创建或将另一个作业排入队列 [中部时间](https://en.wikipedia.org/wiki/Central_Time_Zone). 在此之前，将返回错误“1029， Export daily quota exceeded”。 除每日配额外，没有文件大小上限。
...
您可以通过调用Get Export Lead/Activity/Program Member Jobs来确定当天使用的磁盘总数。 这些端点会返回过去七天中的所有作业的列表。 您可以将该列表过滤到仅包含当天完成的作业(使用 `status` 和 `finishedAt` 属性)。 然后，对这些作业的文件大小求和，以生成使用的总数量。 无法删除文件以回收磁盘空间。
...
当我们创建作业时，它会在 `exportId` 属性。 然后，我们可以使用此作业ID将作业排入队列、取消作业、检查其状态或检索已完成的文件。
...
有时候，您可能必须检索最近的作业。 使用相应对象类型的“获取导出作业”可轻松完成此操作。 每个“获取导出作业”端点支持 `status` 筛选器字段，a  `batchSize` 以限制返回的作业数，并且 `nextPageToken` 用于分页大型结果集。 状态筛选器支持导出作业的每个有效状态：“已创建”、“已排队”、“正在处理”、“已取消”、“已完成”和“失败”。 batchSize的最大值和缺省值均为300。 我们来获取潜在客户导出作业列表：
...
终结点使用进行响应 `status` 过去七天内为结果数组中的该对象类型创建的每个作业的响应。 响应将仅包括发出调用的API用户所拥有的作业的结果。
...
内层 `status` member指示作业的进度，它可以是以下值之一： Created、Queued、Processing、Canceled、Completed、Failed。 在这种情况下，我们的作业已完成，因此我们可以停止轮询并继续检索文件。 完成后， `fileSize` member表示文件的总长度（以字节为单位），并且 `fileChecksum` 成员包含文件的SHA-256哈希。 作业状态在达到“已完成”或“失败”状态后的30天内可用。
...
为了支持提取数据的部分和恢复友好检索，文件端点可以选择支持HTTP标头 `Range` 类型 `bytes` (每 [RFC 7233](https://datatracker.ietf.org/doc/html/rfc7233))。 如果未设置标头，则将返回所有内容。 要检索文件的前10,000字节，您需要将以下标头作为GET请求的一部分传递到端点，从字节0开始：
...
可以部分检索文件，或稍后使用 `Range` 标题。 文件的范围从字节0开始，到值 `fileSize` 减1。 文件的长度也会报告为的值中的分母。 `Content-Range` 调用获取导出文件端点时的响应标头。 如果检索部分失败，可以稍后恢复。 例如，如果您尝试检索长度为1000字节的文件，但只收到前725字节，则可以从失败点重试检索，方法是再次调用端点并传递一个新范围：
...
作业状态端点在 `fileChecksum` 属性时间 `status` 为“已完成”。 校验和是导出文件的SHA-256哈希值。 您可以将校验和与检索文件的SHA-256哈希进行比较，以验证其是否已完成。
