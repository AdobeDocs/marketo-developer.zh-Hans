...
feature: SOAP
description: "describeMObjects SOAP调用"
...
此函数返回标准和虚拟MObject的元数据。 它将Marketo对象作为输入并返回与该对象关联的字段属性。

## 请求

| 字段名称 | 必需/可选 | 描述 |
| --- | --- | --- |
| 对象名称 | 必需 | 要为其获取元数据的对象的名称。 `objectName` 可以是以下任一项： `ActivityRecord`， `LeadRecord`， `Opportunity`， `OpportunityPersonRole` |
