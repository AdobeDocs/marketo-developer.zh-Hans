...
1. 现在添加资源包，如下所示：找到 `MarketoFramework.XCframework` 在项目导航器中打开它，然后在“访达”中打开。 拖放 `MKTResources.bundle` 以复制捆绑包资源。
...
1. 将以下代码粘贴到 `application:didFinishLaunchingWithOptions`：函数。
...
1. 替换 `munkinAccountId` 和 `secretKey` 上面使用的“Munchkin帐户ID”和“密钥”，这两者可在Marketo中找到 **[!UICONTROL Admin]** > **[!UICONTROL Mobile Apps and Devices]** 部分。
...
1. 包含应用程序:openURL:sourceapplication:annotation: 到AppDelegate.m文件(Objective-C)
...
1. 在Firebase控制台上配置Firebase应用程序。
   1. 创建/添加项目 [](https://accounts.google.com/ServiceLogin?passive=1209600&amp;osid=1&amp;continue=https://console.firebase.google.com/&amp;followup=https://console.firebase.google.com/)Firebase控制台。
      1. 在 [Firebase控制台](https://accounts.google.com/ServiceLogin?passive=1209600&amp;osid=1&amp;continue=https://console.firebase.google.com/&amp;followup=https://console.firebase.google.com/)，选择 `Add Project`.
      1. 从现有Google Cloud项目列表中选择GCM项目，然后选择 `Add Firebase`.
      1. 在Firebase欢迎屏幕中，选择 `Add Firebase to your Android App`.
      1. 提供包名称和SHA-1，然后选择 `Add App`. 新 `google-services.json` Firebase应用程序的文件已下载。
      1. 选择 `Continue` 并按照有关在Android Studio中添加Google Services插件的详细说明操作。
   ...
   1. 在Android应用程序中配置FCM更改
      1. 切换到Android Studio中的“项目”视图，查看项目根目录
         1. 将下载的&quot;google-services.json&quot;文件移到Android应用程序模块根目录中
