...
Munchkin可接受各种配置设置以自定义行为。 配置设置是在调用时作为第二个参数传递的JavaScript对象的属性 [Munchkin.init()](lead-tracking.md#munchkin-behavior)
...
| anonymizeip | 布尔值 | 为新访客匿名处理Marketo中记录的IP地址。您可以通过检查您的 `{Munchkin-Id}.mktoresp.com` 域具有以下地址之一： `192.28.144.124` `134.213.193.62` `192.28.147.68` `103.237.104.82`. 或者，您可以从unix shell运行以下脚本： nslookup {munchkin-id}.mktoresp.com | grep -E -c -e “(192.28.144.124,134.213.193.62,192.28.147.68,103.237.104.82)”如果命令输出“0”，则您的订阅不会配置Munchkin V2；如果命令输出1或更大，则配置该命令。 |
| apiOnly | 布尔值 | 如果设置为true，则 `Munchkin.Init()` 函数将不会调用 `visitsWebPage`. 这对于需要完全控制每个页面的单页Web应用程序非常有用 `visitsWebPage` 事件。 |
...
| wsInfo | 字符串 | 采用字符串来定位工作区。 此工作区ID通过在“管理员”>“集成”>“Munchkin”菜单中选择工作区来获取。 此设置仅适用于匿名潜在客户记录的初始创建。 一旦为该潜在客户记录建立了Munchkin Cookie值，将无法使用wsInfo参数更改其分区。 由于此设置仅影响匿名潜在客户，因此它仅与Web报表中特定于分区的匿名访客相关。 |
