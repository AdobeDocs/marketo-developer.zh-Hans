...
通常，响应在结果数组中包含一组更多的字段，但出于演示目的，我们忽略了它们。 结果数组中的每一项都对应于潜在客户记录中可用的字段，并且至少具有id、displayName和数据类型。 对于给定字段，rest和soap子对象可能存在，也可能不存在，它的存在将指示该字段在REST或SOAP API中是否有效。 此 `readOnly` 属性指示字段是否通过相应的API（REST或SOAP）为只读。 length属性指示字段的最大长度（如果存在）。 dataType属性指示字段的数据类型。

## 查询

商机检索的主要方法有两种：按ID获取商机，和按过滤器类型获取商机。 按ID获取潜在客户采用单个潜在客户ID作为路径参数并返回单个潜在客户记录。

或者，您可以传递一个字段参数，其中包含要返回的以逗号分隔的字段名称列表。 如果此请求中未包含字段参数，则会返回以下默认字段： `email`， `updatedAt`， `createdAt`， `lastName`， `firstName`、和 `id`. 在请求字段列表时，如果请求的是特定字段但未返回，则该值默认为空。
...
`filterType` 接受任何自定义字段，或大多数常用字段。 调用 `Describe2` 端点，以获取允许用于的可搜索字段的完整列表 `filterType`. 按自定义字段搜索时，仅支持以下数据类型： `string`， `email`， `integer`. 您可以获取字段详细信息（描述、类型等） 使用上述Describe方法。

`filterValues` 接受最多300个逗号分隔格式的值。 该调用会搜索潜在客户字段与包含的一个字段匹配的记录 `filterValues`. 如果与商机过滤器匹配的商机数量大于1,000，则会返回错误：“1003，有太多结果与过滤器匹配”。
...
此调用将搜索与中包含的ID匹配的记录 `filterValues`，并返回任何匹配的记录。
...
在此请求中，您会看到两个重要字段， `action` 和 `lookupField`.  `action` 指定请求的操作类型，可以 `createOrUpdate`， `createOnly`， `updateOnly`，或 `createDuplicate`. 如果忽略，则该操作将默认为 `createOrUpdate`.  此 `lookupField` parameter指定在操作满足以下任一条件时要使用的键 `createOrUpdate` 或 `updateOnly`. 如果 `lookupField` 省略，默认键为 `email`.

缺省情况下，使用缺省分区。 或者，您可以指定 `partitionName` 参数，仅当操作为 `createOnly` 或 `createOrUpdate`. 对象 `partitionName` 要用作其他重复数据删除标准，该重复数据删除必须是自定义重复数据删除规则中源类型的一部分。 在更新操作期间，如果指定的分区中不存在潜在客户，则会返回错误。 如果仅API用户没有访问指定分区的权限，则会返回错误。

此 `id` 字段只能在使用时作为参数包含 `updateOnly` 操作，作为 `id` 是系统管理的唯一密钥。

请求还必须具有 `input` 参数，是潜在客户记录的数组。 每个潜在客户记录都是一个JSON对象，其中包含任意数量的潜在客户字段。 记录中包含的键对于该记录应该是唯一的，并且所有JSON字符串都应采用UTF-8编码。 此 `externalCompanyId` 字段可用于将潜在客户记录链接到公司记录。 此 `externalSalesPersonId` 字段可用于将潜在客户记录链接到销售人员记录。

注意：当同时或连续快速执行潜在客户更新插入请求时，如果在首次返回之前使用相同的值执行后续调用，则在使用相同的键值发出多个请求时，可能会导致出现重复记录。 这可以通过使用 `createOnly`，或 `updateOnly` 或者，将呼叫排入队列并等待您的呼叫返回，然后再使用相同的键进行后续更新插入呼叫。
...
获取潜在客户字段端点检索潜在客户对象中所有字段的元数据，包括。 默认情况下，最多返回300条记录。 您可以使用 `batchSize` 查询参数以减小此数量。 如果 `moreResult` 属性为true，这意味着有更多的结果可用。 继续调用此端点，直到 `moreResult` 特性返回false，这意味着没有可用的结果。 此 `nextPageToken` 从此API返回的内容应始终重复用于此调用的下一个迭代。
...
所需的输入参数是潜在客户字段对象的数组。 每个对象都包含一个或多个属性。 必需的属性包括 `displayName`， `name`、和 `dataType` 分别对应于字段的UI显示名称、字段的API名称和字段类型。  或者，您可以指定 `description`， `isHidden`， `isHtmlEncodingInEmail`、和 `isSensitive`.

有一些规则与名称和 `displayName` 命名。 name属性必须是唯一的，以字母开头，并且只包含字母、数字或下划线。 此 `displayName` 必须是唯一的，并且不能包含特殊字符。  常见的命名惯例是将骆驼大小写应用于 `displayName` 以生成名称。 例如， `displayName` “我的自定义字段”的将生成名称“myCustomField”。
...
必需 `fieldApiName` path参数指定要更新的字段的API名称。 所需的输入参数是一个包含单个潜在客户字段对象的数组。  字段对象包含一个或多个属性。
...
推送商机是同步到Marketo商机的一种替代方法，它的主要功能是允许比标准同步商机具有更大的触发能力(与Marketo表单的使用方式类似)。 除了同步潜在客户字段之外，此端点还允许根据传递到端点的Cookie值关联潜在客户。 这是通过传递 `mkt_tok` 通过点击Marketo电子邮件或在调用中传递项目名称生成的值。 此端点还会创建单个可触发活动，该活动与Marketo中的项目和/或营销策划相关联。 这允许触发归因于特定活动或项目的商机捕获事件，以从Marketo中启动关联的工作流。

Push Lead界面与Sync Lead非常相似。 所有相同的主键均有效，字段使用相同的API名称（没有操作参数，因为这始终是更新插入操作）。 此 `programName` 和输入参数是必需的，并且 `lookupField`， `source`、和 `reason` 参数是可选的。 输入参数是lead对象的数组。 生成的活动将归属于相应的命名项目。 此 `source` 和 `reason` 参数是任意字符串字段，可添加到请求中以将这些值嵌入到生成的活动中。 这些限制可以用作相应触发器(将潜在客户推送到Marketo)和过滤器(将潜在客户推送到Marketo)中的约束。
...
要传递 `mkt_tok` 参数，将该值分配给输入参数中的潜在客户记录中的mktToken成员，如下所示。
...
提交表单时遵循标准潜在客户数据库模式。 单个对象记录在POST请求的JSON正文的必需输入成员中传递。 必需 `formId` 成员包含目标Marketo表单ID。

可选 `programId` 可用于指定要向其中添加潜在客户的程序，和/或指定要向其中添加程序成员自定义字段的程序。 如果 `programId` 提供将潜在客户添加到计划中，并且还添加表单中存在的任何计划成员字段。 请注意，指定的程序必须与表单位于同一工作区中。 如果表单不包含项目群成员自定义字段和 `programId` 未提供，则潜在客户不会添加到项目中。 如果表单驻留在程序中且 `programId` 如果未提供，则当表单中存在一个或多个项目群成员自定义字段时使用项目。

在输入记录中， `leadFormFields` 对象是必需的。 此对象包含一个或多个与要填充的表单字段对应的名称/值对。  所有指定的字段必须在指定的表单中定义。 该名称是字段的REST API名称。 请注意 `email` 字段为必填项。

此 `visitorData` 成员对象是可选的，包含与页面访问数据对应的名称/值对，包括 `pageURL`， `queryString`， `leadClientIpAddress`、和 `userAgentString`. 可用于填充其他活动字段以进行筛选和触发。
...
有时候，合并重复记录是必要的，Marketo通过合并潜在客户API为此提供了便利。 合并潜在客户将合并其活动日志、项目、营销策划、列表成员资格和CRM信息，并将其所有字段值合并到单个记录中。 Merge Leads将商机ID作为路径参数，并采用一个 `leadId` 作为查询参数，或者 `leadIds` 参数。
...
在path参数中指定的商机是入选商机，因此，如果合并的记录之间有任何字段冲突，则采用来自入选者的值，但入选记录中的字段为空且失败记录中的对应字段为空的除外。 以下任一项中指定的潜在客户： `leadId` 或 `leadIds` 参数是失去的潜在客户。

如果您启用了SFDC-sync的订阅，则还可以使用 `mergeInCRM` 请求中的参数。 如果设置为true，则还会在CRM中执行相应的合并。 如果两个潜在客户都在SFDC中，并且一个是CRM潜在客户，而另一个是CRM联系人，则入选者将成为CRM联系人（无论将哪个潜在客户指定为入选者）。 如果其中一个潜在客户位于SFDC中，而另一个仅位于Marketo中，则入选者为SFDC潜在客户（无论指定哪个潜在客户为入选者）。
...
要通过Marketo UI访问列表ID，请导航到列表。 列表 `id` 在静态列表的URL中， `https://app-****.marketo.com/#ST1001A1`. 在此示例中，1001是 `id` 以取得这份名单。
...
“按潜在客户ID获取列表”端点采用潜在客户记录 `id` path参数并返回商机所属的所有静态列表记录。
...
计划会员资格可采用与列表类似的方式检索。 在调用“按项目ID获取潜在客户”端点并传递 `programId` path参数。
...
响应结构非常相似，因为结果数组中的每个项都是一个潜在客户，只是每个记录还具有一个名为“membership”的子对象。 此成员资格对象包括有关潜在客户与调用中指示的项目之间的关系的数据，始终显示其 `progressionStatus`， `acquiredBy`， `reachedSuccess`、和 `membershipDate`. 如果父计划也是参与计划，则成员资格将具有成员 `stream`， `nurtureCadence`、和 `isExhausted` 表明其在参与计划中的地位和活动。
...
“按潜在客户ID获取程序”端点采用潜在客户记录ID路径参数，并返回该潜在客户所属的所有程序记录。 可选 `filterType` 和 `filterValues` 参数允许您根据程序ID进行筛选。
