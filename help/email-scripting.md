...
- 对于每个自定义对象，每个人员/联系人在运行时都可以使用最近更新的十条记录，并且这些记录按照从最近更新（在0时）到最旧更新（在9时）的顺序排列。 您可以按照说明](https://experienceleague.adobe.com/en/docs/marketo/using/product-docs/administration/email-setup/change-custom-object-retrieval-limits-in-velocity-scripting)增加[可用的记录数。
